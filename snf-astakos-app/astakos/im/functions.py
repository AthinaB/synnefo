...
def validate_project_action(project, action, request_user=None):
    if action == "TERMINATE":
...
def terminate(project_id, request_user=None, reason=None):
    project = get_project_for_update(project_id)
    validate_project_action(project, "TERMINATE", request_user)
...
def suspend(project_id, request_user=None, reason=None):
    project = get_project_for_update(project_id)
    validate_project_action(project, "SUSPEND", request_user)
...
def reinstate(project_id, request_user=None, reason=None):
    get_project_lock()
    project = get_project_for_update(project_id)
    validate_project_action(project, "REINSTATE", request_user)
    project.resume(actor=request_user, reason=reason)
    quotas.qh_sync_project(project)
